...
def drain_queues():
    ...
    while True:
        try:
            ...
            if "bets" in result:
                st.session_state.bets = result["bets"]
...
def qlog(msg, kind=""):
    st.session_state.log_queue.put({"ts": datetime.now().strftime("%H:%M:%S"), "msg": msg, "kind": kind})
...
def render_bet(bet):
    ...
    extra_cls = "hedge" if is_hedge else ("placed" if bet.get("placed") else "")
    st.markdown(f"""<div class="bet-row {extra_cls}">
      ...
      {'&nbsp;&nbsp;<span style="color:var(--warn);font-size:0.72rem">HEDGE</span>' if is_hedge else ''}
...
def _run_bot_thread(cfg, live_mode, max_exp_hours):
    ...
    try:
        ...
        sys.argv = ["trading_bot.py"] + (["--live"] if live_mode else []) + (["--max-expiration-hours", str(max_exp_hours)] if max_exp_hours else [])
...
def _mock_thread(live_mode):
    ...
    st.session_state.result_queue.put({"bets": bets, "stats": {"total_bets":len(bets),"total_amount":sum(b["amount"] for b in bets),"placed":sum(1 for b in bets if b.get("placed")),"hedges":sum(1 for b in bets if b.get("is_hedge"))}, "done": True})
...
with c2:
    ...
    rc2 = "badge-running" if st.session_state.running else "badge-idle"
    st.markdown(f'<div style="text-align:right;padding-top:1.5rem"><span class="badge {ec}">{"DEMO" if use_demo else "LIVE"}</span> <span class="badge {rc2}">{"RUNNING" if st.session_state.running else "IDLE"}</span></div>', unsafe_allow_html=True)
...
cols = st.columns(5)
for col, title, val, sz in zip(cols,
...
with ic:
    if live_mode:
        st.markdown('<div class="warn-box" style="margin-top:1.8rem">🔴 <strong>LIVE MODE:</strong> Real money will be wagered. Verify your credentials and settings.</div>', unsafe_allow_html=True)
    else:
...
with b1:
    if st.button("▶ Run Bot", disabled=st.session_state.running, use_container_width=True):
        ...
        for q in [st.session_state.log_queue, st.session_state.result_queue]:
            while not q.empty():
                try: q.get_nowait()
                except: break
        qlog("Starting Kalshi Deep Trading Bot...", "info")
        qlog(f"Mode: {'LIVE' if live_mode else 'DRY RUN'} | Env: {'DEMO' if use_demo else 'PRODUCTION'}", "info")
...
with b2:
    if st.button("⏹ Stop", disabled=not st.session_state.running, use_container_width=True):
        st.session_state.running = False; qlog("Stopped by user.", "warn"); st.rerun()
with b3:
